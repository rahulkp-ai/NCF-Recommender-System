...
def hit_at_k(
    model: ScratchNCF, val_df, n_items: int, k: int = 10, n_neg: int = 99, seed: int = 0
) -> float:
    """
...
def train(
    ...
    # Build model
    model = ScratchNCF(n_users, n_items, emb_dim=emb_dim, hidden=hidden, seed=seed)
    loss_fn = BCELoss()
    ...
    for epoch in range(1, n_epochs + 1):
        ...
        for u_batch, i_batch, l_batch in batch_iter(users, items, labels, batch_size, rng):
            batch_loss = 0.0
            model.zero_grad()

            for u, i, y in zip(u_batch, i_batch, l_batch, strict=False):
                # Forward
        ...
        if epoch % eval_every == 0:
            hit_k = hit_at_k(
                model,
                val_df.head(200),  # 200 users for speed
                n_items,
                k=10,
            )
    ...
    print("Log saved to evaluation/scratch_ncf_log.json")

    # Save model weights
    np.savez(
        LOG_DIR / "scratch_ncf_weights.npz",
        user_emb=model.user_emb.W,
        item_emb=model.item_emb.W,
        **{f"W{i}": layer.W for i, layer in enumerate(model.linears)},
        **{f"b{i}": layer.b for i, layer in enumerate(model.linears)},
    )
